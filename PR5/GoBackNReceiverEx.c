...
if (data->hasErrors){
//do something
DEBUGOUT("ERROR receiving packet %d !!!!\n", 1);
} else if (data->seqNo == lastReceivedSeqNo+1) { //the one we are expecting
sendAck(s, data, ++lastReceivedSeqNo+1);
DEBUGOUT("Ack sent for packet with seqNo %d, lastReceivedSeqNo: %d\n", data->seqNo, lastReceivedSeqNo);
if (data->size == sizeof(*data)){
fclose(output);
DEBUGOUT("END. totalBytes: %d, goodBytes: %d\n", totalBytes, goodBytes);
freeGoBackNMessageStruct(data);
sleep(0.1);
break;
} else {
...
} else if (data->seqNo <= lastReceivedSeqNo) { //an older one
DEBUGOUT("Older packet received with seqNo: %ld, lastReceivedSeqNo: %ld\n", data->seqNo, lastReceivedSeqNo);
sendAck(s, data, lastReceivedSeqNo+1);
} else if (data->seqNo > lastReceivedSeqNo+1) { //too new
DEBUGOUT("Packet too new received with seqNo: %ld, lastReceivedSeqNo: %ld\n", data->seqNo, lastReceivedSeqNo);
sendAck(s, data, lastReceivedSeqNo+1);
}
