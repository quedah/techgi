...
if (ack->hasErrors){
DEBUGOUT("ERROR receiving ack !!!!\n");
} else if (lastAckSeqNo >= ack->seqNoExpected-1) { //delayed ack
//do nothing??
DEBUGOUT("Delayed ack received, with seqNo: %ld, lastAckSeqNo: %ld\n", ack->seqNoExpected-1, lastAckSeqNo);
} else {
freeBuffer(dataBuffer, getFirstSeqNoOfBuffer(dataBuffer), ack->seqNoExpected-1);
lastAckSeqNo = ack->seqNoExpected-1;
                DataPacket * data = getDataPacketFromBuffer(dataBuffer, lastAckSeqNo+1);
if (data){
timerExpiration = data->timeout;
}
if (nextSendSeqNo < ack->seqNoExpected)
nextSendSeqNo = ack->seqNoExpected;
DEBUGOUT("ACK Received: %ld. New nextSendSeqNo: %ld, first seq buffer %ld\n", lastAckSeqNo, nextSendSeqNo, getFirstSeqNoOfBuffer(dataBuffer));
}
            /* YOUR TASK:
